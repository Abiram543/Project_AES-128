...
//Control signals from Controller//
    ...
    input wire SR_sel, ISR_sel, MC_sel, IMC_sel, Data_sel, mode_sel, out_sel, key_sel, ARK_key_sel, IARK_key_sel,
...
/*Temporory wires*/
wire [127:0] Din_reg, Dout_reg, key_in_reg, key_out_reg, state_in, state_reg;
...
//SubBytes//
SubBytes U2 (.A(SB_A), .mode(mode_out), .Q(SB_Q));
...
assign ARK_key = ARK_key_sel ? key_out_reg : 'b0;   // Depends on the ARK_key_sel signal (from the AES_Controller), selects the key value. (This is for Encryption)
...
assign IARK_A = (IARK_sel == 2'd1) ? Dout_reg :   // Decryption: round0: Data in --> AddRoundKey
                ...
                'b0;    
assign IARK_key = IARK_key_sel ? key_out_reg : 'b0;     // Key selection based on the IARK_key_sel (This is for Decryption)
